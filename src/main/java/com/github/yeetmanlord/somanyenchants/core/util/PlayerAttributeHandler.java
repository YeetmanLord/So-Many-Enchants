					...
					if (!multimap.isEmpty()) {
						...
						for (Entry<Attribute, AttributeModifier> entry : multimap.entries()) {
							...
							if (flag) {
								...
								list1.add((new TranslationTextComponent("attribute.modifier.plus." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslationTextComponent(entry.getKey().getDescriptionId()))).withStyle(TextFormatting.BLUE));
							}
							else if (d0 < 0.0D) {
								d1 = d1 * -1.0D;
								list1.add((new TranslationTextComponent("attribute.modifier.take." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslationTextComponent(entry.getKey().getDescriptionId()))).withStyle(TextFormatting.RED));
							}
				...
				if (list.size() > 0) {
					...
					if (x != null) {
						...
						if (ModEnchantmentHelper.hasEnchant(EnchantmentInit.ATTACK_REACH.get(), stack)) {
							event.getToolTip().add(x, (new StringTextComponent(" " + String.valueOf(ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(playerIn.getAttributeBaseValue(AttributeInit.ATTACK_DISTANCE.get()))) + " ").append(new TranslationTextComponent(AttributeInit.ATTACK_DISTANCE.get().getDescriptionId())).withStyle(TextFormatting.DARK_GREEN)));
						}

						if (ModEnchantmentHelper.hasEnchant(EnchantmentInit.BLOCK_REACH.get(), stack)) {
							event.getToolTip().add(x, (new StringTextComponent(" " + String.valueOf(ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(playerIn.getAttributeBaseValue(ForgeMod.REACH_DISTANCE.get()))) + " Block reach distance")).withStyle(TextFormatting.DARK_GREEN));
						}
					...
					if (stack.getItem() instanceof ArmorItem) {
						...
						for (EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
							...
							if (!multimap.isEmpty()) {
								...
								for (Entry<Attribute, AttributeModifier> entry : multimap.entries()) {
									...
									if (d0 > 0.0D && !flag) {
										list2.add((new TranslationTextComponent("attribute.modifier.plus." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1 + addon), new TranslationTextComponent(entry.getKey().getDescriptionId()))).withStyle(TextFormatting.BLUE));
									}
									else if (d0 < 0.0D && !flag) {
										d1 = d1 * -1.0D;
										list2.add((new TranslationTextComponent("attribute.modifier.take." + attributemodifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1 + addon), new TranslationTextComponent(entry.getKey().getDescriptionId()))).withStyle(TextFormatting.RED));
									}
						...
						if (level > 0 && z != null) {
							event.getToolTip().add(z, (new TranslationTextComponent("attribute.modifier.plus.0", ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(level * 2d), new TranslationTextComponent(Attributes.MAX_HEALTH.getDescriptionId()))).withStyle(TextFormatting.BLUE));
						}
