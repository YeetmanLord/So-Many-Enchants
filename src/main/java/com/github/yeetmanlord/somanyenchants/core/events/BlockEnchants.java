...
public class BlockEnchants {
	...
	public static void breakEnchantedBlock(Block block, Block checkBlock, BlockState state, BlockPos pos, Item drop, World world, PlayerEntity player, Enchantment ench) {
	...
	public static void breakTrappedEnchantedChest(Block block, BlockState state, BlockPos pos, World world, PlayerEntity player) {
								...
								if (item instanceof BlockItem) {
									...
									if (block instanceof ShulkerBoxBlock) {
										...
										if (living instanceof PlayerEntity) {
											...
											living.setItemSlot(EquipmentSlotType.MAINHAND, newStack);
										}

										CommandSource source = command.getSource();

										if (entities.size() == 1) {
											source.sendSuccess(new TranslationTextComponent("commands.enchant.success.single", enchant.getFullname(1), entities.iterator().next().getDisplayName()), true);
										}
										else {
											source.sendSuccess(new TranslationTextComponent("commands.enchant.success.multiple", enchant.getFullname(1), entities.size()), true);
										}
								...
								if (item instanceof BlockItem) {
									...
									if (block instanceof AbstractFurnaceBlock) {
										...
										if (newStack.isEnchanted()) {
											...
											if (entities.size() == 1) {
												source.sendSuccess(new TranslationTextComponent("commands.enchant.success.single", enchant.getFullname(level), entities.iterator().next().getDisplayName()), true);
											}
											else {
												source.sendSuccess(new TranslationTextComponent("commands.enchant.success.multiple", enchant.getFullname(level), entities.size()), true);
											}
