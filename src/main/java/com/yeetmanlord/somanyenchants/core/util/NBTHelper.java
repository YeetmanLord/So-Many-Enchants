...
public class NBTHelper 
{
	public static void renderCustomAttributeLore(PlayerEntity player, double extraVal, Attribute at, String name)
	{
		...
		StringNBT extraValNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf(MathUtils.roundNearestPlace(extraVal + at.getDefaultValue(), -1)) + " " + name + "\"}");
		if(MathUtils.roundNearestPlace(extraVal + at.getDefaultValue(), -1) == MathUtils.roundNearestPlace(extraVal + at.getDefaultValue(), 0))
		{
			extraValNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf((int) MathUtils.roundNearestPlace(extraVal + at.getDefaultValue(), 0)) + " " + name + "\"}");
		}
	...
	public static void renderAttributeLore(PlayerEntity player)
	{
		...
		if(tag.contains("display", 10))
		{
			...
			if(displayList.contains("Lore", 9))
			{
				...
				{
					StringNBT attackDamageNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf(MathUtils.roundNearestPlace(attackDamage, -1)) + " Attack Damage\"}");
					if(MathUtils.roundNearestPlace(attackDamage, -1) == MathUtils.roundNearestPlace(attackDamage, 0))
					{
						attackDamageNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf((int) MathUtils.roundNearestPlace(attackDamage, 0)) + " Attack Damage\"}");
					}
				...
				{
					StringNBT attackSpeedNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf(MathUtils.roundNearestPlace(attackSpeed, -1)) + " Attack Speed\"}");
					if(MathUtils.roundNearestPlace(attackSpeed, -1) == MathUtils.roundNearestPlace(attackSpeed, 0))
					{
						attackSpeedNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf((int) MathUtils.roundNearestPlace(attackSpeed, 0)) + " Attack Speed\"}");
					}	
	...
	public static double getAttackDamage(PlayerEntity player)
	{
		...
		if(effect != null)
		{
			attackDamage -= (effect.getAmplifier() + 1) * 3;
		}
		
		double sharpness = 0;
		int lvl = ModEnchantmentHelper.getSharpnessLevel(player);
		if(lvl > 0)
		{
			sharpness += 0.5 + (lvl * 0.5);
		}
	...
	public static void updateAttributeLore(ItemStack stack, PlayerEntity player)
	{
		...
		if(tag.contains("display", 10))
		{
			...
			if(display.contains("Lore", 9))
			{
				...
				{
					String s = lore.getString(x);
					if(s.contains(" Attack Speed\"}") && attackSpeed != player.getBaseAttributeValue(Attributes.ATTACK_SPEED))
					{
					...
					if(s.contains(" Attack Damage\"}") && attackDamage  != player.getBaseAttributeValue(Attributes.ATTACK_DAMAGE))
					{
			...
			if(display.contains("Lore", 9))
			{
				...
				for(int x = 0; x < lore.size(); x++)
				{
					...
					if(s.contains(" " + displayName + "\"}"))
					{
						...
						lore.remove(x);
						StringNBT newAttrNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf(MathUtils.roundNearestPlace(attrValue, -1)) + " " + displayName + "\"}");
						if(MathUtils.roundNearestPlace(attrValue, -1) == MathUtils.roundNearestPlace(attrValue, 0))
						{
							newAttrNBT = StringNBT.valueOf("{\"text\": \" \u00a72" + String.valueOf((int) MathUtils.roundNearestPlace(attrValue, 0)) + " " + displayName + "\"}");
						}
