...
public class EnchantedHopperTileEntity extends LockableLootTileEntity implements IHopper, ITickableTileEntity {
	...
	private NonNullList<ItemStack> inventory = NonNullList.withSize(5, ItemStack.EMPTY);
	   ...
	   public EnchantedHopperTileEntity() {
	      super(TileEntityTypeInit.ENCHANTED_HOPPER.get());
	   }
	   ...
	   private boolean updateHopper(Supplier<Boolean> p_200109_1_) {
	      if (this.world != null && !this.world.isRemote) {
	         if (!this.isOnTransferCooldown() && this.getBlockState().get(EnchantedHopper.ENABLED)) {
	            boolean flag = false;
	            if (!this.isEmpty()) {
	   ...
	   private boolean transferItemsOut() {
	      if (Capabilities.insertHook(this)) return true;
	      IInventory iinventory = this.getInventoryForHopperTransfer();
	      if (iinventory == null) {
	         return false;
	      } else {
	         Direction direction = this.getBlockState().get(EnchantedHopper.FACING).getOpposite();
	   ...
	   private static ItemStack insertStack(@Nullable IInventory source, IInventory destination, ItemStack stack, int index, @Nullable Direction direction) {
	      ...
	      if (canInsertItemInSlot(destination, stack, index, direction)) {
	         ...
	         if (flag) {
	            if (flag1 && destination instanceof EnchantedHopperTileEntity) {
	               EnchantedHopperTileEntity EnergizedHopperTileEntity1 = (EnchantedHopperTileEntity)destination;
	               if (!EnergizedHopperTileEntity1.mayTransfer()) {
	                  int k = 0;
	                  if (source instanceof EnchantedHopperTileEntity) {
	                     EnchantedHopperTileEntity EnergizedHopperTileEntity = (EnchantedHopperTileEntity)source;
	                     if (EnergizedHopperTileEntity1.tickedGameTime >= EnergizedHopperTileEntity.tickedGameTime) {
	                        k = 1;
	                     }
	                  }

	                  EnergizedHopperTileEntity1.setTransferCooldown(2 - k);
	               }
	            }
	   ...
	   private IInventory getInventoryForHopperTransfer() {
	      Direction direction = this.getBlockState().get(EnchantedHopper.FACING);
	   ...
	   public boolean mayTransfer() {
	      return this.transferCooldown > 2;
	   }
	   ...
	   protected Container createMenu(int id, PlayerInventory player) {
	      return new EnchantedHopperContainer(id, player, this);
	   }
