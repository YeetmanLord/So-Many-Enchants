...
public abstract class LivingEntity extends Entity {
   ...
   public void setRevengeTarget(@Nullable LivingEntity livingBase, LivingEntity living) 
   {
   ...
   public boolean attackEntityFrom(DamageSource source, float amount) {
      ...
      if (this.isInvulnerableTo(source)) {
         ...
         if ((float)this.hurtResistantTime > 10.0F) {
            if (amount <= this.getLastDamage()) {
               return false;
            }

            this.damageEntity(source, amount - this.getLastDamage());
            this.setLastDamage(amount);
            flag1 = false;
         } else {
            this.setLastDamage(amount);
   ...
   public void blockUsingShieldPublic(LivingEntity living)
   {
	   living.constructKnockBackVector(this);
   }
   ...
   public boolean checkTotemDeathProtectionPublic(DamageSource damageSourceIn) {
	      if (damageSourceIn.canHarmInCreative()) {
	         return false;
	      } else {
	         ItemStack itemstack = null;
   ...
   public boolean canBlockDamageSource(DamageSource damageSourceIn, LivingEntity living) 
   {
       return living.canBlockDamageSource(damageSourceIn);
   }
   ...
   public void onDeath(DamageSource cause, LivingEntity living) {
	      if (net.minecraftforge.common.ForgeHooks.onLivingDeath(living, cause)) return;
	      if (!living.removed && !living.dead) {
	         Entity entity = cause.getTrueSource();
	         LivingEntity livingentity = living.getAttackingEntity();
	         if (living.scoreValue >= 0 && livingentity != null) {
	            livingentity.awardKillScore(living, living.scoreValue, cause);
	         }
	         ...
	         living.dead = true;
	         living.getCombatTracker().reset();
	         if (this.world instanceof ServerWorld) {
	            if (entity != null) {
	               entity.onKillEntity((ServerWorld)living.world, living);
	            }

	            living.spawnDrops(cause);
	            living.createWitherRose(livingentity);
	         }

	         living.world.setEntityState(this, (byte)3);
   ...
   public void applyKnockback(float strength, double ratioX, double ratioZ, LivingEntity living)
   {
	   living.applyKnockback(strength, ratioX, ratioZ);
   }
   ...
   public SoundEvent getHurtSound(DamageSource damageSourceIn, LivingEntity living)
   {
	   return living.getHurtSound(damageSourceIn);
   }
   ...
   @Nullable
   public SoundEvent getDeathSound(LivingEntity living)
   {
...
private boolean canGoThroughtTrapDoorOnLadder(BlockPos pos, BlockState state) {
   ...
   public void damageArmor(DamageSource damageSource, float damage, LivingEntity living) 
   {
	   living.damageArmor(damageSource, damage);
   }

   protected void damageShield(float damage) {
   }
   
   public void damageShield(float damage, LivingEntity living)
   {
	   living.damageShield(damage);
   }
   ...
   public float applyArmorCalculations(DamageSource source, float damage, LivingEntity living)
   {
	   return living.applyArmorCalculations(source, damage);
   }
   ...
   public float applyPotionDamageCalculations(DamageSource source, float damage, LivingEntity living)
   {
	   return living.applyPotionDamageCalculations(source, damage);
   }
   ...
   public float getSoundVolume(LivingEntity living)
   {
   ...
   public float getSoundPitch(LivingEntity living) 
   {
	      return living.getSoundPitch();
   }
...
private void updateElytra() {
   ...
   public void markVelocityChanged(LivingEntity living)
   {
	   living.velocityChanged = this.rand.nextDouble() >= this.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE);
   }
   
   public void setRecentlyHit(int time)
   {
	   this.recentlyHit = time;
   }
